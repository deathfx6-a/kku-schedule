...
function renderUpNext(now = new Date()) {
  ...
  els.upNext.hidden = !slot;
  if (!slot) return;
  els.upNext.classList.toggle("is-live", Boolean(current));
  els.upNextLabel.textContent = t(current ? "upNowLabel" : "upNextLabel");
  els.upNextTitle.textContent = slotTitle(slot);
  applyDepartmentBadge(els.upNextDept, slot.departmentId);
  els.upNextWhen.textContent = current
    ? tf("upEndsIn", { rel: relTime(slotMoment(current, current.range.end), now) })
    : upNextWhen(next, now);
  const room = cleanText(slot.raw.room);
  fillParts(els.upNextMeta, [
    clockRangeLabel(slot.range),
    room ? `${t("roomLabel")} ${room}` : "",
    pick(slot.raw, "instructor")
  ]);
  const then = current && next && next.dateISO === current.dateISO ? next : null;
  els.upNextThen.hidden = !then;
  if (then) els.upNextThen.textContent = tf("upThen", { title: slotTitle(then), time: clockLabel(then.range.start) });
}
