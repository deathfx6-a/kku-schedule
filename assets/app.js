...
const CONFIG = Object.freeze({
  ...
  toastMs: 2600,
  // Calendar export (.ics). KKU is in Abha: Saudi Arabia has no daylight saving.
  minGapMinutes: 30,
  ics: { timeZone: "Asia/Riyadh", utcOffset: "+0300", tzName: "AST", reminderMinutes: 15 },
  languages: ["ar", "en"],
  defaultLanguage: "ar",
...
const state = {
  ...
  editMode: false,          // pencil / add buttons are shown only while editing
  error: null,
  retry: null
};
...
const I18N = {
  ar: {
    ...
    customizeNone: "فعّل «وضع التعديل» لتعديل أي موعد أو الإضافة في يوم بعينه. تعديلاتك تُحفظ على هذا الجهاز فقط.",
    editModeOn: "وضع التعديل",
    editModeOff: "إنهاء التعديل",
    more: "المزيد",
    ...
    upEndsIn: "تنتهي {rel}",
    upStartsIn: "تبدأ {rel}",
    upTodayAt: "اليوم الساعة {time}",
    upTomorrowAt: "غدًا الساعة {time}",
    upDayAt: "{day} الساعة {time}",
    upThen: "بعدها: {title} الساعة {time}",
    addToCalendar: "أضفه لتقويم جوالك",
    icsExported: "نُزّل ملف التقويم: {n} موعدًا قادمًا بتنبيه قبل {m} دقيقة. افتحه ليُضاف إلى تقويمك.",
    icsNothing: "لا مواعيد قادمة لتصديرها.",
    checkAllDay: "تحديد محاضرات {day} كلها كمنجزة",
    uncheckAllDay: "إلغاء تحديد محاضرات {day}",
    ...
    hourForms: { zero: "ساعة", one: "ساعة", two: "ساعتان", few: "ساعات", many: "ساعة", other: "ساعة" },
    minuteForms: { zero: "دقيقة", one: "دقيقة", two: "دقيقتان", few: "دقائق", many: "دقيقة", other: "دقيقة" },
    durationJoin: " و",
    freeLabel: "وقت فراغ",
    freeHave: "لديك {d} فراغ",
    freeUntilNext: "ثم تبدأ محاضرتك",
    gapFree: "فراغ {d}",
  ...
  en: {
    ...
    customizeNone: "Turn on edit mode to change any slot or add one on a given day. Your changes are saved on this device only.",
    ...
    upNextLabel: "Up next",
    upEndsIn: "Ends {rel}",
    upStartsIn: "Starts {rel}",
    upTodayAt: "Today at {time}",
    upTomorrowAt: "Tomorrow at {time}",
    upDayAt: "{day} at {time}",
    upThen: "Then: {title} at {time}",
    addToCalendar: "Add to your phone calendar",
    icsExported: "Calendar file downloaded: {n} upcoming slots with a {m}-minute reminder. Open it to add them to your calendar.",
    icsNothing: "There are no upcoming slots to export.",
    checkAllDay: "Mark all of {day}'s lectures as completed",
    uncheckAllDay: "Unmark {day}'s lectures",
    ...
    hourForms: { one: "hour", other: "hours" },
    minuteForms: { one: "minute", other: "minutes" },
    durationJoin: " ",
    freeLabel: "Free time",
    freeHave: "You have {d} free",
    freeUntilNext: "then your next lecture starts",
    gapFree: "{d} free",
...
function unitForm(count, formsKey) {
  ...
  // Arabic marks one/two in the noun itself ("ساعتان"), so the digit is dropped.
  return state.lang === "ar" && (category === "one" || category === "two") ? form : `${formatNumber(count)} ${form}`;
}

// 140 -> "ساعتان و20 دقيقة" / "2 hours 20 minutes"
function formatDuration(minutes) {
  const hours = Math.floor(minutes / 60);
  const mins = minutes % 60;
  const parts = [];
  if (hours) parts.push(unitForm(hours, "hourForms"));
  if (mins || !hours) parts.push(unitForm(mins, "minuteForms"));
  return parts.join(t("durationJoin"));
}
...
function weekHijriText(week) {
  if (!week.firstDate || !state.data || !state.data.university.showHijriDates) return "";
  try {
    const formatter = new Intl.DateTimeFormat(t("hijriLocale"), { day: "numeric", month: "long" });
    if (week.lastDate && week.lastDate !== week.firstDate && typeof formatter.formatRange === "function") {
      return formatter.formatRange(isoToDate(week.firstDate), isoToDate(week.lastDate));
    }
    return formatter.format(isoToDate(week.firstDate));
  } catch (_) {
    return "";
  }
}
...
function renderHeader() {
  ...
  const credit = pick(university, "designer");
  els.creditText.textContent = credit;
  els.creditLine.hidden = !credit;
  const instagram = cleanText(university.designerInstagram, 60);
  if (instagram) {
    els.creditInstagram.href = `https://instagram.com/${encodeURIComponent(instagram.replace(/^@/, ""))}`;
    setField(els.creditInstagram, "handle", `@${instagram.replace(/^@/, "")}`);
  }
  els.creditInstagram.hidden = !instagram;
...
function renderWeekTabs() {
  ...
  const tabs = state.model.weeks.map((week, index) => {
    ...
    if (week.firstDate) tab.title = formatDateRange(week.firstDate, week.lastDate, { day: "numeric", month: "long" });
  ...
  if (els.todayBtn) els.todayBtn.hidden = tabs.length === 0;
}
...
function renderWeek() {
  ...
  const hijri = weekHijriText(week);
  els.weekHijri.textContent = hijri;
  els.weekHijri.hidden = !hijri;
...
function renderDay(day, dayIndex = 0) {
  ...
  setField(card, "date", day.dateISO ? formatDate(day.dateISO, { day: "numeric", month: "long" }) : (day.raw.date || ""));
  ...
  const checkAll = field(card, "checkAll");
  if (checkAll) {
    const ids = dayCoreIds(day);
    checkAll.hidden = ids.length < 2; // one lecture already has its own checkbox
    checkAll.dataset.checkDay = String(dayIndex);
    updateCheckAllButton(checkAll, day);
  }
  ...
  day.slots.forEach(slot => {
    list.append(renderSlot(slot));
    const gap = slot.range && gaps.find(item => item.after === slot);
    if (gap) list.append(renderGap(gap));
  });
  return card;
}

// "09:00" -> "9:00": the leading zero is noise in a narrow time column.
function clockLabel(minutes) {
  return `${Math.floor(minutes / 60) % 24}:${pad2(minutes % 60)}`;
}

function fillTime(el, text, range) {
  const parts = range ? [clockLabel(range.start), clockLabel(range.end)] : text.split(/\s*[-–—]\s*/);
  if (parts.length === 2) {
    const start = document.createElement("span");
    const end = document.createElement("span");
    start.textContent = parts[0];
    end.textContent = parts[1];
    start.className = "slot-time-start";
...
function renderSlot(slot) {
  if (slot.isBreak) {
    ...
    fillTime(field(row, "time"), slot.time, slot.range);
...
function updateProgressViews() {
  ...
  renderUpNext(now);
  els.weekDays.querySelectorAll("[data-check-day]").forEach(button => {
    const day = dayAt(Number(button.dataset.checkDay));
    if (day) updateCheckAllButton(button, day);
  });
}
...
function exportCustom() {
  ...
  downloadFile(`kku-schedule-backup-${todayISO()}.json`, JSON.stringify(payload, null, 2), "application/json");
...
/* ===== Study tools ===== */
// Small helpers around the schedule: what's on now/next, ticking a whole day,
// jumping to today, the edit-mode switch, and exporting to a phone calendar.
...
function dayAt(index) {
  const week = currentWeek();
  return week ? week.days[index] || null : null;
}

// Empty stretches between two slots of the same day, long enough to be useful.
function dayGaps(day) {
  const timed = day.slots.filter(slot => slot.range).sort((a, b) => a.range.start - b.range.start);
  const gaps = [];
  for (let i = 0; i < timed.length - 1; i += 1) {
    const start = timed[i].range.end;
    const end = timed[i + 1].range.start;
    if (end - start >= CONFIG.minGapMinutes) gaps.push({ start, end, after: timed[i], before: timed[i + 1] });
  }
  return gaps;
}

function renderGap(gap) {
  const row = cloneTemplate("tpl-gap");
  fillTime(field(row, "time"), "", { start: gap.start, end: gap.end });
  setField(row, "length", tf("gapFree", { d: formatDuration(gap.end - gap.start) }));
  return row;
}

function dayCoreIds(day) {
  return day.slots.filter(slot => slot.isCore && slot.id).map(slot => slot.id);
}

function updateCheckAllButton(button, day) {
  const ids = dayCoreIds(day);
  const allDone = ids.length > 0 && ids.every(isCompleted);
  const label = tf(allDone ? "uncheckAllDay" : "checkAllDay", { day: dayName(day) });
  button.setAttribute("aria-pressed", String(allDone));
  button.setAttribute("aria-label", label);
  button.title = label;
}

// Ticks every core lecture of the day; if they are all ticked already, unticks them.
function toggleWholeDay(index) {
  const day = dayAt(index);
  const ids = day ? dayCoreIds(day) : [];
  if (!ids.length) return;
  const markDone = !ids.every(isCompleted);
  ids.forEach(id => {
    if (markDone) state.checklist[id] = true;
    else delete state.checklist[id];
  });
  saveChecklist();
...
/* --- Up next --- */
// Every timed, non-break slot of the group, oldest first (cached per model).
function timedSlots() {
  if (!state.model) return [];
  if (!state.model.timed) {
    const list = [];
    state.model.weeks.forEach(week => week.days.forEach(day => day.slots.forEach(slot => {
      if (!slot.isBreak && slot.dateISO && slot.range) list.push(slot);
    })));
    list.sort((a, b) => (a.dateISO === b.dateISO ? a.range.start - b.range.start : a.dateISO < b.dateISO ? -1 : 1));
    state.model.timed = list;
  }
  return state.model.timed;
}

function slotMoment(slot, minutes) {
  const [year, month, day] = slot.dateISO.split("-").map(Number);
  return new Date(year, month - 1, day, Math.floor(minutes / 60), minutes % 60);
}

function findUpNext(now) {
  let current = null;
  let next = null;
  for (const slot of timedSlots()) {
    const start = slotMoment(slot, slot.range.start);
    if (!current && start <= now && now < slotMoment(slot, slot.range.end)) current = slot;
    else if (start > now) { next = slot; break; }
  }
  return { current, next };
}
...
function upNextWhen(slot, now) {
  const start = slotMoment(slot, slot.range.start);
  const time = clockLabel(slot.range.start);
  const today = todayISO(now);
  if (slot.dateISO === today) {
    return (start - now) / 60000 <= 90 ? tf("upStartsIn", { rel: relTime(start, now) }) : tf("upTodayAt", { time });
  }
  if (slot.dateISO === addDaysISO(today, 1)) return tf("upTomorrowAt", { time });
  return tf("upDayAt", { day: formatDate(slot.dateISO, { weekday: "long", day: "numeric", month: "long" }), time });
}

// "13:00–14:15" wrapped in a left-to-right isolate so an Arabic line can't flip it.
function clockRangeLabel(range) {
  return `\u2066${clockLabel(range.start)}–${clockLabel(range.end)}\u2069`;
}

function fillSlotCard(titleEl, whenEl, metaEl, slot, whenText) {
  titleEl.textContent = slotTitle(slot);
  whenEl.textContent = whenText;
  const room = cleanText(slot.raw.room);
  fillParts(metaEl, [
    clockRangeLabel(slot.range),
    room ? `${t("roomLabel")} ${room}` : "",
    pick(slot.raw, "instructor")
  ]);
}

// The free window between "now" and the next lecture, when it is worth naming.
function freeGapNow(now, next) {
  if (!next || next.dateISO !== todayISO(now)) return null;
  const minutes = Math.round((slotMoment(next, next.range.start) - now) / 60000);
  return minutes >= CONFIG.minGapMinutes ? { minutes, until: next.range.start } : null;
}

function renderUpNext(now = new Date()) {
  if (!els.upNext || !state.model) return;
  const { current, next } = findUpNext(now);
  els.upNext.hidden = !current && !next;
  if (els.upNext.hidden) return;

  // 1. The lecture running right now, with how long is left of it.
  els.upNowCard.hidden = !current;
  if (current) {
    fillSlotCard(els.upNowTitle, els.upNowWhen, els.upNowMeta, current,
      tf("upEndsIn", { rel: relTime(slotMoment(current, current.range.end), now) }));
  }

  // 2. Free time — only when nothing is running and the wait is long enough.
  const gap = current ? null : freeGapNow(now, next);
  els.upFreeCard.hidden = !gap;
  if (gap) {
    els.upFreeTitle.textContent = tf("freeHave", { d: formatDuration(gap.minutes) });
    els.upFreeRange.textContent = `\u2066${clockLabel(Math.round((now.getHours() * 60 + now.getMinutes())))} → ${clockLabel(gap.until)}\u2069`;
  }

  // 3. What comes next — shown alongside the current lecture, not instead of it.
  els.upNextCard.hidden = !next;
  if (next) {
    fillSlotCard(els.upNextTitle, els.upNextWhen, els.upNextMeta, next, upNextWhen(next, now));
  }
}

/* --- Today --- */
function jumpToToday() {
  if (!state.model || !state.model.weeks.length) return;
  const index = pickInitialWeek(state.model.weeks);
  if (index !== state.weekIndex) setWeek(index);
  const smooth = !(window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)").matches);
  requestAnimationFrame(() => {
    const tab = els.weekTabs.querySelector(".week-tab.is-active");
    if (tab) tab.scrollIntoView({ block: "nearest", inline: "center" });
    const target = els.weekDays.querySelector(".day-card.is-today") || els.weekPanel;
...
function applyEditMode() {
  ...
  if (els.editModeBtn) {
    ...
    els.editModeBtn.classList.toggle("is-on", state.editMode);
  }
  if (els.editModeLabel) els.editModeLabel.textContent = t(state.editMode ? "editModeOff" : "editModeOn");
}
...
function closeMoreMenu() {
  if (els.moreMenu && els.moreMenu.open) els.moreMenu.open = false;
}
...
// Lines longer than 75 octets must be folded; Arabic letters are 2 bytes each.
...
function icsLocal(dateISO, minutes) {
  return `${dateISO.replace(/-/g, "")}T${pad2(Math.floor(minutes / 60))}${pad2(minutes % 60)}00`;
}

function buildIcs(slots) {
  const { timeZone, utcOffset, tzName, reminderMinutes } = CONFIG.ics;
  const university = state.data.university;
  const calendarName = [pick(university, "siteTitle") || t("appName"), university.shortName].filter(Boolean).join(" · ");
  const stamp = new Date().toISOString().replace(/[-:]/g, "").replace(/\.\d{3}/, "");
  const lines = [
    "BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//KKU Schedule//Personal Timetable//AR", "CALSCALE:GREGORIAN", "METHOD:PUBLISH",
    `X-WR-CALNAME:${icsEscape(calendarName)}`, `X-WR-TIMEZONE:${timeZone}`,
    "BEGIN:VTIMEZONE", `TZID:${timeZone}`, "BEGIN:STANDARD", "DTSTART:19700101T000000",
    `TZOFFSETFROM:${utcOffset}`, `TZOFFSETTO:${utcOffset}`, `TZNAME:${tzName}`, "END:STANDARD", "END:VTIMEZONE"
  ];
  slots.forEach(slot => {
    const title = slotTitle(slot);
    const typeLabel = pick(slot.type, "label");
    const summary = typeLabel && typeLabel !== title ? `${title} (${typeLabel})` : title;
    const subjectName = pick(slot.subject, "name");
    const description = [
      subjectName && subjectName !== title ? subjectName : "",
      pick(slot.raw, "instructor"),
    ...
    lines.push(
      "BEGIN:VEVENT",
      `UID:${slot.id || uid()}-${slot.dateISO}@kku-schedule`,
      `DTSTAMP:${stamp}`,
      `DTSTART;TZID=${timeZone}:${icsLocal(slot.dateISO, slot.range.start)}`,
      `DTEND;TZID=${timeZone}:${icsLocal(slot.dateISO, slot.range.end)}`,
      `SUMMARY:${icsEscape(summary)}`
    );
    if (room) lines.push(`LOCATION:${icsEscape(room)}`);
    if (description) lines.push(`DESCRIPTION:${icsEscape(description)}`);
    if (reminderMinutes > 0) {
      lines.push("BEGIN:VALARM", "ACTION:DISPLAY", `DESCRIPTION:${icsEscape(summary)}`, `TRIGGER:-PT${reminderMinutes}M`, "END:VALARM");
    }
...
// Exports what is still ahead (with the student's own edits applied).
function exportIcs() {
  const now = new Date();
  const upcoming = timedSlots().filter(slot => slotMoment(slot, slot.range.end) > now);
  if (!upcoming.length) { showToast(t("icsNothing")); return; }
  downloadFile(`kku-schedule-${todayISO(now)}.ics`, buildIcs(upcoming), "text/calendar;charset=utf-8");
  showToast(tf("icsExported", { n: formatNumber(upcoming.length), m: formatNumber(CONFIG.ics.reminderMinutes) }));
}

function bindStudyTools() {
  on("todayBtn", "click", jumpToToday);
  on("editModeBtn", "click", toggleEditMode);
  on("icsBtn", "click", exportIcs);
  on("weekDays", "click", event => {
    const button = event.target.closest("[data-check-day]");
    if (button) toggleWholeDay(Number(button.dataset.checkDay));
  });
  // Choosing an item closes the menu; so does a click anywhere else or Escape.
  on("moreMenu", "click", event => { if (event.target.closest(".more-item")) closeMoreMenu(); });
  document.addEventListener("click", event => {
    if (els.moreMenu && els.moreMenu.open && !els.moreMenu.contains(event.target)) closeMoreMenu();
  });
